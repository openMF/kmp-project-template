#!/usr/bin/env bash
# scripts/health/project-health.sh — project health / sanity harness.
#
# Reads gradle/fork.properties (the project-level SINGLE SOURCE OF TRUTH) and runs every
# scripts/health/checks/*.sh against it. This is the ONE place a fork learns whether its
# customization is complete + correct: signing/org identity re-forked, store listing authored,
# required keys present. Runs in CI (quality-gate.yml) and at the end of customizer.sh so a
# fresh fork gets an immediate sanity report. Pure bash — no Gradle, no network.
#
...
#
# project-health exit: 0 when no check FAILs (WARNs allowed) · 1 when any check FAILs.
set -uo pipefail

HEALTH_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"   # …/scripts/health
...
app_id="$(grep -E '^\s*applicationId\s*=' "$HEALTH_ROOT/gradle/libs.versions.toml" 2>/dev/null | head -1 | sed 's/.*=[[:space:]]*//; s/"//g')"
[ -n "$app_id" ] || app_id="?"
echo "── ${C_BLD}Project Health${C_RST} · ${org} ${C_DIM}(${app_id})${C_RST} ──"
