...
interface ScreenWithMutationStream<R, W> {
     ...
     * [template.core.base.store.screen.FetchPolicy.CACHE_ONLY]. Mirrors
     * [template.core.base.store.screen.ScreenDataStream.refresh].
     */
    fun refresh()
     ...
     * [template.core.base.store.submit.SubmitHandler], surfacing progress and
     * outcome in [CombinedState.mutation].
     *
     * Idempotent — additional calls while a submission is in flight are dropped
     * silently (the underlying [template.core.base.store.submit.SubmitHandler]
     * enforces this).
     */
     ...
     * [template.core.base.store.submit.SubmitState.Idle]. Called by screens
